...
function Tag({ children, green }: { children: React.ReactNode; green?: boolean }) {
  return (
    <span className={`${TAG_STYLES} ${green ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-700"}`}>
      {children}
    </span>
  );
}
...
export default function CampCard({ camp }: Props) {
  ...
  return (
    <article className="bg-white rounded-2xl p-5 flex flex-col gap-4 border border-slate-200 hover:border-blue-400 hover:shadow-md transition-all">
      {/* Header */}
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-xs text-slate-500 mb-1">{camp.prefecture} · {camp.area}</p>
          <Link href={`/camp/${camp.slug}`} className="text-lg font-bold text-slate-900 hover:text-blue-500 transition-colors leading-tight">
            {camp.name}
          </Link>
        </div>
        <div className="shrink-0 text-right">
          <div className="text-2xl font-bold text-blue-500">{camp.soloScore.toFixed(1)}</div>
          <div className="text-xs text-slate-500">ソロ評価</div>
        </div>
      </div>
