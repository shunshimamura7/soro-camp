...
export type Campground = {
   ...
   *
   * 実際に `hadano-togawa-camp` は、このフラグが立ったまま座標が5km先の松田町を指していた。
   *
   * 内訳（2026-08-14 時点・153件）:
   *   - cc751ab 以降に座標が動いた 70件 … 目視ツール（`scripts/coord-tool.html`）由来。**信頼できる**
   *   - cc751ab で立ったまま不動   83件 … **推定で付いたまま。裏付けが無い**
   *
   * `validate-data.js` が「`coordsVerified` が true なのに機械検証を通っていない」を警告する。
   * 位置が確定して誤っているものだけを1件ずつ出し、残りは件数のサマリにしてある
   * （全部出すと毎回83行になり、警告が読まれなくなるため）。
   *
   * **true を「人が確認した」の意味で使わないこと。**使ってよいのは
   * apply スクリプトに実ピンの記録が残っている分だけ（§14〜§18）。
   */
  coordsVerified?: boolean;
  /**
   * scores（5軸）を人が評価したか。
   *
   * **`false` は「評価していない」であって「低い」ではない。**
   * 料金だけ一次情報で取れて設備を見ていない段階のレコードは、全軸に 3 を置くが
   * それは中庸の評価ではなく**未評価の置き値**。
   *
   * **未指定は「不明」。**`priceVerified` を priceNote の有無から機械的に付けた
   * 9fd15e3 の轍を踏まないよう、無いものを「評価済み」と推定しない。
   * `validate-data.js` の value 帯検査は**明示的に `false` のときだけ**飛ばす。
   */
  scoresVerified?: boolean;
  /**
   * 国土地理院の逆ジオコーディングを通過したなら true。次の3つだけを意味する。
   *
   *   1. 座標が海上・湖面でない（逆ジオが市区町村を返す）
   *   2. 返ってきた都道府県が `prefecture` と一致する
   *   3. 返ってきた市区町村が `address` の文字列に現れる
   *
   * **3 は 2026-08-13 に足した。**それ以前は都道府県しか見ておらず、
   * `yadoriki-camp`（address 松田町 / 逆ジオ 山北町）のように
   * **市町村が違っても同じ県なら true が立っていた**（引き継ぎ §6-11・§17-4-1）。
   * `kabutomushi-mori-camp` が拾えていたのは、たまたま県境をまたいでいたからにすぎない。
   *
   * **通過しても地点が正しいとは限らない。**同じ大字の別地点でも通るし、
   * 大字（`lv01Nm`）までは見ていない。そこは scripts/verify-address-gsi.js の担当。
   * **`coordsVerified`（人の目視）の代わりには絶対にならない。**
   *
   * 検査が成立しなかった場合の扱い。
   *   - **`address` が空なら通過しない**（判定は `NO_ADDRESS`）。
   *     照合していないものに「機械検証済み」が立つのは、上で直した穴と同じ構造になる
   *   - 政令市の区違い（`WARD_MISMATCH`）は**通過する。**
   *     区の再編（浜松市の南区→中央区、2024年）で一斉に発生し、
   *     座標の正しさが変わっていないのにフラグだけ落ちるため、合否には使わない。
   *     区の食い違いは scripts/coord-city-check-2026-08.md に人が見る材料として出る
   *
   * scripts/verify-coords-gsi.js が判定の元になる。**手で書かないこと。**
   */
