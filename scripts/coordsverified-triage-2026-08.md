...
生成: 2026-08-13 / `node scripts/coordsverified-triage.js`（このファイルは毎回上書き）
...
| ③-T1 自治体ぐるみで lv01 欠落 | 11 | 低優先（距離は代表点まで） |
...
| `kofu-shinrinyoku-hiroba` | active | 甲府市 | 黒平町 | 17.7km | 距離 17.7km に説明がつかない（address が市区町村までで、大字の突き合わせ自体ができない） |
| `tsuchimura` | active | 静岡市清水区 | 西里 | 12km | 距離 12km に説明がつかない（address が市区町村までで、大字の突き合わせ自体ができない） |
...
| `fuji-midori-kyuka-auto` | active | 鳴沢村 | − | 3.42km | 鳴沢村 は全2件で lv01 が欠落。距離 3.42km は町の代表点からの距離でしかない |
