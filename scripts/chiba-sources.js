 ...
 * レコード0件の大字は測定対象にすらならなかった（既存3県でも実測で穴が出ていた）。
 * **千葉は接続を検討していた当時 `data/campgrounds.json` にレコードが1件も無かった**ので、
 * その頃に走らせると**全地区がこの穴に落ちた。**同じ穴を千葉にもう一度掘ることになる。
 *
 ...
 *
 * **接続は定義の追加であって実行ではない。**`--all` の対象がこのファイルのキーの分だけ増え、
 * 既存の市区町村のソースは1本も変わらない（`.check-chiba-connect.js` で実測）。
 *
 * ## ⚠ 件数をこのコメントに書かないこと
 *
 * レコード数・県別の内訳・登録市区町村数のような**可変値は、書いた瞬間から腐る**
 * （引き継ぎ §18-3「消費側が古い前提を持ったまま静かに間違い続ける」/ §20-14）。
 * ここに書いてよいのは**登録の設計判断と、外の事実**（ソースの構造・層・取得できない理由）だけ。
 * 現在値は `data/campgrounds.json` と `Object.keys(MUNI_SOURCES)` から**毎回数える**。
 *
 * ## ★ 千葉だけ層が薄い
 *
