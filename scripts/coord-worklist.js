 ...
 */
const HOLD = {
  // 2026-08-13 時点で空。**空でも仕掛けごと消さない**（保留の置き場と自己診断は残す）。
  //
  // 消した保留の記録（自己診断が「もう要らない保留」と出したため削除。§18-3 の腐り防止）:
  // - sankoso-auto … worklist の行から外れ、保留として参照されなくなっていた。
  //   施設名の疑義（三光荘/山光荘）自体は電話21件・引き継ぎ §7 側で追う
  // - saiko-tsuhara-camp … 同上。住所の割れ（西湖351/2299）は §7 側で追う
};
 ...
 * `coordsverified-triage.js` の同名関数と同じ修正）。
 */
