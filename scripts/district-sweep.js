...
function splitAddress(addr) {
  ...
  // ★ 郡の抽出は**市・区をまたがない**（2026-08-17 に踏んだ）。
  //
  // 旧: /^(.{1,6}?郡)/ は「千葉県木更津市**下郡**1886」で
  //     `木更津市下郡` を郡名として食い、**city が null になって地区外に落ちた。**
  //     ETOWA KISARAZU（エトワ木更津）がこれで b2 に消えた。
  //
  // **「内」バグ（`堀之内`→`堀之`）と同じ family。**
  // 地名の一部を、接尾語や区切りとして書いた正規表現が食う型。
  //
  // 郡は住所の中で必ず市区町村より**前**に来るので、
  // 郡名の途中に `市` や `区` が現れることはない。そこで除外文字にする。
  // **`町`・`村` は除外しない。**「田村郡」「北村山郡」のように
  // **郡名そのものに村が入る**実例があり、除外すると今度はそちらが取れなくなる。
  //
  // 既存192件で判定が変わるものは **0件**（2026-08-17 実測）。
  ...
  //
  // **末尾の除去は「敷地内」「地内」「地先」だけ（2026-08-16 修正）。**
  // 以前は `/(?:地内|地先|先|内)$/` で**裸の「内」「先」まで食っていた。**
  // 意図は位置を表す接尾語を落とすことで、**大字の最後の1文字を削ることではない。**
  //
  //   千葉県夷隅郡大多喜町堀之内595 → 大字が `堀之内` ではなく **`堀之`** になっていた
  //   （`市原市堀之内` でも同じ。大多喜SABO の住所で踏んだ）
  //
  // 害は表示の崩れだけではない。`inDistrict` は大字の前方一致なので、
  // **`堀之丸` が `堀之内` の地区に入ってしまう**（`.mock-oaza-suffix-test.js` の対照）。
  // 大字が「内」で終わるものはいくらでもあるが、「地内」「地先」で終わる大字は無い。
...
/**
 * 都留市観光協会「つるのルーツ発掘巡り」の**キャンプ**タグ一覧。
 *
 * ## 見つけ方（上野原と同じ型。詳細ページ → パンくず → カテゴリ一覧）
 *
 * 入口は L2 の会員一覧に出ていた大沢オートキャンプ場の詳細
 * `https://tsurukankou.jp/discovery/919/`。**その詳細ページのタグから1枚上に上がった。**
 * サイト内検索・観光トップ・sitemap のカテゴリ一覧のどれからも辿り着けない。
 *
 * ## ★ 2026-08-15 の探索が「キャンプの一覧は無い」と結論した理由と、その誤り
 *
 * 前回は `category-sitemap.xml` と `discoverycat-sitemap.xml` の2本だけを数えて
 * 「キャンプのカテゴリは存在しない」と結論していた（`sweep-tsuru-2026-08.md` L1-B）。
 * **カテゴリ（discoverycat）にキャンプが無いのは事実。だがタグ（discoverytag）にはある。**
 * `sitemap.xml` はタクソノミー系を8本持っていて、そのうち
 * **`discoverytag-sitemap.xml` を見ていなかった**（44タグ・その1つが「キャンプ」）。
 *
 * **「無い」は、見たところに無かっただけのことがある。**
 * 一覧の不在を結論にするときは、**タクソノミーを1本ずつ潰した記録を残すこと**（§6-7 の同型）。
 *
 * ## 中身（実測・7件）
 *
 * キャンプ場でないものが混ざる。タグは施設分類ではなく「キャンプに関係する」の意味で付く。
 *
 *   THE FOREST（フォレスト）        … グランピング。テント持込なし
 *   芭蕉　月待ちの湯                … 日帰り温泉。戸沢の森和みの里の併設施設
 *   宝の山　ふれあいの里　キャンプ場/ネイチャーセンター
 *   Berry Park in FISH ON!鹿留      … 釣り場併設
 *   大沢オートキャンプ場
 *   鹿留オートキャンプ場
 *   すげのレジャー                  … 釣り堀併設
 *
 * ## 詳細ページは末尾スラッシュが要る
 *
 * 一覧の href は `/discovery/919`（スラッシュなし）だが、**それは 301 で
 * `/discovery/919/` に飛ぶ。**`list()` の中で正規化しておく
 * （リダイレクト追従の有無を取得層に依存させない）。
 *
 * ## 住所を取るときの注意
 *
 * 基本情報の表 `<th>住所</th><td>〒402-0032 山梨県都留市鹿留2961 …` から取る。
 * **本文の下に Instagram / YouTube の埋め込みがあり、そこにも別施設の住所が入る**
 * （実測: 大沢の詳細ページに FISH ON!鹿留 の「鹿留1543」が出てくる）。
 * `<th>住所</th>` に固定して**最初の1件だけ**を取ること。本文の素通し検索をしてはいけない。
 *
 * ## ★ 掲載漏れ検出専用。実在／営業の根拠に使わない（2026-08-18 実測）
 *
 * **この一覧は閉館を反映しない。**`takaranoyama-fureai`（宝の山ふれあいの里）は
 * **施設公式が 2026-02-02 に閉館を告知し、都留市公式も閉館を明記している**のに、
 * ここの詳細ページ（`/discovery/1045/`）は**営業中の記述のまま**で閉館に触れていない。
 * 判定の全文は `scripts/ghost-verdict-2026-08.md` §12。
 *
 * **鮮度だけでなく網羅率でも保証にならない。**実測 0%（`sweep-都留市.md`）。
 * ただし分母＝実在確実レコードが1件しかないので、**この 0% も精度の証拠にはならない**
 * （§18-7。母集団とセットでしか意味を持たない）。
 *
 * **使ってよいのは MISSING の検出だけ。**
 * ここに載っていることを `status` を `active` にする根拠にしない（§6-22「L1 掲載は営業の証明にならない」）。
 * ここに載っていないことを `closed` にする根拠にもしない（§6-7・ORPHAN は閾値未満で判定不能）。
 */
const SRC_TSURU_KANKO = {
  id: 'tsuru-kankou',
  layer: 'L1',
  kind: 'listDetail',
  label: '都留市観光協会 つるのルーツ発掘巡り キャンプ',
  note: 'タグ一覧。キャンプ場以外（温泉・グランピング）が混ざる前提',
  pages: ['https://tsurukankou.jp/discovery-tag/%E3%82%AD%E3%83%A3%E3%83%B3%E3%83%97/'],
  list(html) {
    const seen = new Map();
    const re =
      /<a href="(https:\/\/tsurukankou\.jp\/discovery\/\d+)\/?"[^>]*class="tl-Contents__category__listItem"[\s\S]*?class="tl-Contents__category__listItem__title"[^>]*>([^<]+)</g;
    let m;
    while ((m = re.exec(html))) {
      if (!seen.has(m[1])) seen.set(m[1], cleanText(m[2]));
    }
    // 末尾スラッシュ無しは 301。取得層に追従を任せない
    return [...seen].map(([u, name]) => ({ name, url: u + '/' }));
  },
  address(html) {
    const m = html.match(/<th[^>]*>\s*住所\s*<\/th>\s*<td[^>]*>([^<]+)/);
    return m ? tidyAddress(m[1].replace(/〒\d{3}-?\d{4}/, '')) : null;
  },
};
...
/* ── 千葉県を接続する（2026-08-16。案Cのあと）─────────────────────────
 *
 * `chiba-sources.js` は**案Cが決まるまで接続しない**約束で寝かせてあった。
 * 理由は「地区がレコード由来だと、レコード0件の千葉は全地区が126件の穴に落ちる」。
 * **案Cで地区は `MUNI_SOURCES` のキー由来になったので、その前提が消えた。**
 *
 * ## 定義の追加であって、実行ではない
 *
 * `--all` の対象が 18 → 26 に増えるだけ。**既存18市町村の `sources` には触らない。**
 * 千葉のキーは8つとも既存と衝突しない（接続時に実測）。
 *
 * ## `jalan` / `napCamp` を渡しているのは循環参照のため
 *
 * `chiba-sources.js` はこのファイルのヘルパを使う。相互参照になるので、
 * **向こうが `require('./district-sweep.js')` した時点では `module.exports` がまだ空。**
 ...
 *
 * ## b2 の意味は案C（2026-08-16）で変わった
 *
 * **旧**（大字単位の地区）: じゃらん等は市単位で取るが、地区は「北杜市高根町清里」のような
 * 大字単位だったので、**同じ市の別の大字が必ず b2 に落ちた。**
 * それが b2 の大半で、**正常な b2 が支配的**だった。実測 7,374件のうち 2,014件がこの型。
 *
 * **新**（市区町村単位の地区）: **「市は同じで大字が違う」という状態が存在しない。**
 * 地区＝市区町村なので、市が一致すれば必ず地区内。
 * したがって **b2 は全件が「市区町村ごと違う」**になる（A の実測で 366件 / うち市区町村も違う 366件）。
 *
 * **分け方（`dropSameCity`）は残してある。**0件であることが
 * 「市町村単位になっている」ことの直接の確認になるから。**0件を「測れていない」と読まないこと。**
 *
 * ## それでも b2 の大半は正常
 *
 * 理由が変わっただけで結論は同じ。**じゃらんは市全体、なっぷ・hinata は広域、
 * ウォーカープラスとキャンナビは県全体**を返す。
 * 県単位の一覧を1市町村の地区に当てれば、**残りの市町村ぶんは必ず b2 に落ちる。**
 * 疑うのは件数ではなく、**その市区町村の住所を持つのに地区外に落ちたもの**（いまは定義上0件）。
 * ========================================================================== */
...
function renderMd(ctx) {
  ...
  /* ---- 大字が取れないソース項目の行き先（案C ステップ1）---------------- */
  {
    ...
    L.push('');
    if (nb.total) {
      // ★ 分類名は必ずバックティックで囲む。**素で書いてはいけない。**
      //
      // 集計側（`.baseline-before-planc.js` / `.after-planc.js`）は
      //   /^\|\s*\*?\*?(MISSING|IN_DATA|ORPHAN)[^|]*\|\s*\*?\*?(\d+)/gm
      // で冒頭のサマリ表を拾う。この表を素の `| MISSING | 52 |` で書くと
      // **本文の後ろにあるこちらが最後に一致して、サマリの値を上書きする。**
      // 道志村で実測: 全体スキャン MISSING 37 / IN_DATA 28、正しくは 42 / 15。
      //
      // **同じ型を踏むのは2回目。**§5 のパーサが引用ブロック内の手書き表を拾った件と同じで、
      // 「md を正規表現で読む」設計は**後から節を足すたびに壊れうる。**
      // 集計側は冒頭のサマリ表だけを見るように直したが、
      // **md 側でも当たらない形にしておく**（どちらか一方だけだと次に足す人が同じ穴を作る）。
      L.push('| 落ちた先 | 件数 | 意味 |');
      L.push('|---|---:|---|');
      const MEAN = {
  ...
  /* ---- 出力に載らなかったソース側の項目 ------------------------------- */
  {
    ...
    L.push('');
    L.push('**⚠ b2 の大半は正常。**理由は案C（2026-08-16）で変わった。');
    L.push('');
    L.push('**旧**（大字単位の地区）は「同じ市の別の大字」が b2 の大半だった。');
    L.push('**いまは地区＝市区町村なので、その状態が存在しない。**市が一致すれば必ず地区内。');
    L.push('');
    L.push('残るのは**広域ソースの取りこぼし**。じゃらんは市全体、なっぷ・hinata は広域、');
    L.push('ウォーカープラスとキャンナビは**県全体**を返すので、');
    L.push('**他の市町村ぶんは必ずここに落ちる。**件数の大きさ自体は異常の根拠にならない。');
    L.push('');
    L.push('**疑うのは「この市区町村の住所を持つのに地区外に落ちたもの」**＝下の b2-b。');
    L.push('**案C後は定義上0件になる。**0件を「測れていない」ではなく');
    L.push('**「市町村単位になっていることの確認」**と読むこと。');
    L.push('');
