...
### 実在が未確認の施設には施設固有の断定描写を書かない

`needsVerify: true`（実在・正式名称が確認できていない）施設では、
その施設について断定的に書かない。断り書きだけに留める。

**地名や周辺施設の事実であっても、その施設と結びつける形では書かない。**
事実が正しくても、その施設がそこにある裏付けがなければ、
読者は「確認済みの施設が、確認済みの場所にある」と受け取ってしまう。

```
✗ ※この施設の実在・正式名称を確認中です。西伊豆堂ヶ島近く、駿河湾に沈む夕陽が絶品。
   トンボロ現象が見られる奇景の地。
   → 堂ヶ島のトンボロ現象は事実だが、そこにこの施設がある裏付けはない
○ ※この施設の実在・正式名称を確認中です。

✗ ※施設名・所在地を確認中です。山梨最南端・南部町の渓流沿い、人が来ない穴場の極致。
○ ※施設名・所在地を確認中です。
```

施設情報（`features` や `cautions`）に裏付けのある記述であれば残してよい。
例: 予約方法、営業期間、焚き火の可否など、データとして持っている内容。

検出は以下で行う。定期的に実行して増えていないか確認する。

```bash
node scripts/check-unverified-descriptions.js   # → scripts/unverified-descriptions.md
```

自動では書き換えない。文面の判断は人が行う。
`※` で始まる文は編集上の断り書きとして検出対象から除外される。
